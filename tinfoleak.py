...
class User:
    ...
    def set_user_information(self, api):
        try:
            ...
            if td.days > 0:
                self.tweets_average = round(
                    float(self.statuses_count / (td.days * 1.0)), 2
                )
                self.likes_average = round(
                    float(api.favourites_count / (td.days * 1.0)), 2
                )
            else:
            ...
            try:
                ...
                if api.entities["description"]["urls"]:
                    ...
                    expanded_url = api.entities["description"]["urls"][0][
                        "expanded_url"
                    ]
                    self.expanded_description = tmp_expanded_description.replace(
                        url, expanded_url
                    )
                else:
                    self.expanded_description = ""
            ...
            try:
                if api.profile_banner_url:
                    self.profile_banner_url = str(api.profile_banner_url).replace(
                        "_normal", ""
                    )
                else:
                    self.profile_banner_url = ""
    ...
    def set_global_information(self):
        try:
            ...
            for s in self.sources:
                self.sources_percent[s[0]] = round(
                    (self.sources_count[s[0]] * 100.0) / self.sources_total_count, 1
                )
...
class Lists:
    ...
    def get_memberships(self, client, listed_count, screen_name):
        try:
            ...
            username_directory = (
                os.path.dirname(os.path.abspath(__file__)) + "/" + screen_name
            )
            ...
            cursor = -1
            api_path = (
                "https://api.twitter.com/1.1/lists/memberships.json?screen_name="
                + screen_name
            )
                        ...
                        if "Rate limit exceeded" in str(response_dictionary):
                            ...
                            for public_list in response_dictionary["lists"]:
                                if public_lists < int(str(ui.tb_lists_number.text())):
                                    public_lists += 1
                                    show_ui_message(
                                        str(public_lists) + " public lists analyzed",
                                        "INFO",
                                        br=0,
                                    )
                                    ...
                                    cursor.movePosition(
                                        QtGui.QTextCursor.StartOfLine, 0
                                    )
                                    cursor.movePosition(
                                    ...
                                    csvWriter.writerow(
                                        [
                                            ...
                                            public_list["member_count"],
                                            public_list["subscriber_count"],
                                            public_list["uri"],
                                            public_list["user"]["screen_name"],
                                            public_list["user"]["created_at"],
                                            public_list["user"]["name"],
                                            public_list["user"]["description"],
                                            public_list["user"]["followers_count"],
                                            public_list["user"]["friends_count"],
                                        ]
                                    )
                                    csvFile.flush()
                                else:
            ...
            show_ui_message(
                "Output file: " + username_directory + "/" + memberships_file,
                "INFO",
                br=1,
            )
    ...
    def get_ownerships(self, client, screen_name):
        try:
            ...
            cursor = -1
            api_path = (
                "https://api.twitter.com/1.1/lists/ownerships.json?screen_name="
                + screen_name
            )
                            ...
                            for owner_list in response_dictionary["lists"]:
                                ...
                                csvWriter.writerow(
                                    [
                                        ...
                                        owner_list["id_str"],
                                        owner_list["name"],
                                        owner_list["description"],
                                        owner_list["member_count"],
                                        owner_list["subscriber_count"],
                                        owner_list["uri"],
                                        owner_list["user"]["screen_name"],
                                        owner_list["user"]["created_at"],
                                        owner_list["user"]["name"],
                                        owner_list["user"]["description"],
                                        owner_list["user"]["followers_count"],
                                        owner_list["user"]["friends_count"],
                                    ]
                                )
                                csvFile.flush()
            ...
            show_ui_message(
                str(owner_lists) + " public lists owned by the user", "INFO", br=1
            )
            show_ui_message(
                "Output file: " + username_directory + "/" + ownerships_file,
                "INFO",
                br=1,
            )
    ...
    def get_lists(self, client, screen_name):
        try:
            ...
            cursor = -1
            api_path = (
                "https://api.twitter.com/1.1/lists/list.json?screen_name=" + screen_name
            )
                    ...
                    try:
                        ...
                        csvWriter.writerow(
                            [
                                ...
                                user_list["subscriber_count"],
                                user_list["uri"],
                                user_list["user"]["screen_name"],
                                user_list["user"]["created_at"],
                                user_list["user"]["name"],
                                user_list["user"]["description"],
                                user_list["user"]["followers_count"],
                                user_list["user"]["friends_count"],
                            ]
                        )
                        csvFile.flush()
                    except Exception as e:
            ...
            show_ui_message(
                "User subscribed to " + str(user_lists) + " lists", "INFO", br=1
            )
            show_ui_message(
                "Output file: " + username_directory + "/" + lists_file, "INFO", br=1
            )
...
class Collections:
    ...
    def get_collections(self, client, screen_name):
        try:
            ...
            cursor = -1
            api_path = (
                "https://api.twitter.com/1.1/collections/list.json?screen_name="
                + screen_name
            )
                    ...
                    if "Rate limit exceeded" in str(response_dictionary):
                        ...
                        if response_dictionary["objects"]:
                            for timeline in response_dictionary["objects"]["timelines"]:
                                collections += 1
                                name = response_dictionary["objects"]["timelines"][
                                    str(timeline)
                                ]["name"]
                                try:
                                    description = response_dictionary["objects"][
                                        "timelines"
                                    ][str(timeline)]["description"]
                                except Exception as e:
                                    description = ""
                                url = response_dictionary["objects"]["timelines"][
                                    str(timeline)
                                ]["collection_url"]

                                csvWriter.writerow(
                                    [collections, timeline, name, description, url]
                                )
                                csvFile.flush()

                        if len(response_dictionary["objects"]) > 0:
                            cursor = response_dictionary["response"]["cursors"][
                                "next_cursor"
                            ]
                        else:
                            cursor = 0
            ...
            show_ui_message(
                str(collections) + " public collections owned by the user", "INFO", br=1
            )
            show_ui_message(
                "Output file: " + username_directory + "/" + collections_file,
                "INFO",
                br=1,
            )
...
class Activity:
            ...
            if tweet.entities["urls"]:
                ...
                for m in medias:
                    try:
                        url = m["expanded_url"]
                        if url:
                            expanded_url = urllib.request.urlopen(url)
                            if (
                                "https://twitter.com/i/web/status/"
                                not in expanded_url.url
                            ):
    ...
    def set_global_information(self):
        try:
            ...
            self.activity_tweet_percent = round(
                (self.activity_tweet * 100.0) / self.activity_count, 1
            )
            self.activity_retweet_percent = round(
                (self.activity_retweet * 100.0) / self.activity_count, 1
            )
            self.activity_url_percent = round(
                (self.activity_url * 100.0) / self.activity_count, 1
            )
            self.activity_media_percent = round(
                (self.activity_media * 100.0) / self.activity_count, 1
            )
...
class Followers:
    ...
    def get_followers(self, username, api, limit):
        try:
            ...
            username_directory = (
                os.path.dirname(os.path.abspath(__file__)) + "/" + username
            )
            if not os.path.isdir(username_directory):
                os.mkdir(username_directory)
            ...
            analyzed_user = 0
            for userid in list(tweepy.Cursor(
                api.followers_ids, screen_name=username
            ).items()):
                try:
                    if int(analyzed_user) < int(limit):
                        ...
                        show_ui_message(
                            str(analyzed_user) + "/" + str(limit) + " users analyzed",
                            "INFO",
                            br=0,
                        )
            ...
            show_ui_message(
                "Output file: " + pics_directory + "/" + followers_file, "INFO", br=1
            )
...
class Friends:
    ...
    def get_friends(self, username, api, limit):
        try:
            ...
            analyzed_user = 0
            for userid in list(tweepy.Cursor(api.friends_ids, screen_name=username).items()):
                try:
                    if int(analyzed_user) < int(limit):
                        user = api.get_user(userid)
                        analyzed_user += 1
            ...
            show_ui_message(
                "Output file: " + pics_directory + "/" + friends_file, "INFO", br=1
            )
        ...
        try:
            ...
            for m in medias:
                ...
                if socialnetwork.lower().find("instagram") >= 0:
                    ...
                    #: ----------------------------------------------
                    urls = re.search(
                        '"viewer_has_saved_to_collection":(.*)"profile_pic_url":"(.*)","username":"(.*)","blocked_by_viewer"',
                        html,
                    )
                    ...
                    if socialnetwork.lower().find("foursquare") >= 0:
                        ...
                        if urls:
                            ...
                            urls = re.search(
                                'canonicalPath":"..(\w*)","canonicalUrl"', html
                            )
                            if urls:
                                username = urls.group(1)
                                link = "https://foursquare.com/" + username
                        tmp = re.search(
                            '<div class="venue push"><h1><strong>(.*)</strong> at ',
                            html,
                        )
                        if tmp:
                        ...
                        tmp = re.search(
                            '<div id="mapContainer"><img src="(.*)" alt="(.*)" class="avatar mainUser"width="86"',
                            html,
                        )
                        if tmp:
                        ...
                        if socialnetwork.lower().find("facebook") >= 0:
                            ...
                            if status.source.lower().find("facebook") >= 0:
                                ...
                                if not html:
                                    ...
                                    if urls:
                                        ...
                                        if urls:
                                            ...
                                            try:
                                                ...
                                                pass
                                            tmp = re.search(
                                                '<img class="profilePic img" alt="(.*)" src="(.*)" /></a></div></div><div class="_58gk">',
                                                html,
                                            )
                                            if tmp:
                                                pic = tmp.group(2).replace("&amp;", "&")
                                            tmp = re.search(
                                                '<span itemprop="name">(.*)</span><span class="_5rqt">',
                                                html,
                                            )
                                            if tmp:
                                    ...
                                    urls = re.search(
                                        'autocomplete="off" name="next" value="https://www.facebook.com/(.*)/posts/[0-9]*"',
                                        html,
                                    )
                                    if urls:
                                        ...
                                        try:
                                            response = urllib.request.urlopen(
                                                "http://longurl.org/expand?url=" + url
                                            )
                                            html2 = response.read()
                                            urls = re.search(
                                                '<a href="https://www.facebook.com/(.*)/posts/[0-9]*">https://',
                                                html2,
                                            )
                                        except Exception as e:
                                            pass
                                ...
                                if self.see_again == 0:
                                    username = ""
                                if (
                                    self.user_sn[status.user.screen_name][1][0]
                                    and self.user_sn[status.user.screen_name][1][
                                        0
                                    ].find("Unknown")
                                    < 0
                                    and self.see_again > 0
                                ):
                                    ...
                                    try:
                                        ...
                                        pass
                                    urls = re.search(
                                        '<ul class="social"><li><a href="http://www.facebook.com/profile.php\?id=(.*)" rel="nofollow" target="_blank" class="fbLink"',
                                        html,
                                    )
                                    if urls:
                                        username = urls.group(1)
                                        ...
                                        try:
                                            ...
                                            pass
                                        tmp = re.search(
                                            '<img class="profilePic img" alt="(.*)" src="(.*)" /></div></div><meta itemprop="image"',
                                            html,
                                        )
                                        if tmp:
                                            pic = tmp.group(2).replace("&amp;", "&")
                                        tmp = re.search(
                                            'autocomplete="off" name="next" value="https://www.facebook.com/(.*)" /></form>',
                                            html,
                                        )
                                        if tmp:
                                            ...
                                            link = "https://facebook.com/" + username
                                        tmp = re.search(
                                            '<span id="fb-timeline-cover-name">(.*)</span></a><span class="_1xim">',
                                            html,
                                        )
                                        if tmp:
                                            name = tmp.group(1)
                            ...
                            if socialnetwork.lower().find("linkedin") >= 0:
                                ...
                                if not html:
                                    try:
                                        ...
                                        tmp = re.search(
                                            'linkedin.com/in/(.*)"/><link rel="stylesheet"',
                                            html,
                                        )
                                        if tmp:
                                ...
                                if socialnetwork.lower().find("runkeeper") >= 0:
                                    ...
                                    if urls:
                                        ...
                                        try:
                                            ...
                                            pass
                                        tmp = re.search(
                                            'https://graph.facebook.com(.*)" title',
                                            html,
                                        )
                                        if tmp:
                                            pic = (
                                        ...
                                        tmp = re.search(
                                            '<h1 id="userStatementHead"><b>(.*) is using Runkeeper</b>',
                                            html,
                                        )
                                        if tmp:
                                            name = tmp.group(1).decode("utf-8")
                                    ...
                                    if socialnetwork.lower().find("flickr") >= 0:
                                        ...
                                        #: ----------------------------------------------
                                        urls = re.search(
                                            '"og:url" content="https://www.flickr.com/photos/(.*)/[0-9]*/"',
                                            html,
                                        )
                                        if urls:
                                            username = urls.group(1)
                                        ...
                                        if tmp:
                                            pic = (
                                                tmp.group(1).decode("unicode-escape")
                                                + ".jpg"
                                            )
                                        ...
                                        if socialnetwork.lower().find("vine") >= 0:
                                            ...
                                            if urls:
                                                username = urls.group(1)
                                            urls = re.search(
                                                '"url": "https://vine.co/u/(.*)"', html
                                            )
                                            if urls:
                                                link = (
                                                    "https://vine.co/u/" + urls.group(1)
                                                )
                                            if username:
                                                # Stop after the first result
                                                break
                                            ...
                                            if (
                                                socialnetwork.lower().find("periscope")
                                                >= 0
                                            ):
                                                # Periscope
                                                #: ----------------------------------------------
                                                ...
                                                if (
                                                    socialnetwork.lower().find("kindle")
                                                    >= 0
                                                ):
                                                    # Kindle
                                                    #: ----------------------------------------------
                                                    urls = re.search(
                                                        'customerId":"(.*)","howLongAgo',
                                                        html,
                                                    )
                                                    if urls:
                                                        tmp = urls.group(1)
                                                        try:
                                                            url = (
                                                                "https://kindle.amazon.com/profile/redirect/"
                                                                + tmp
                                                            )
                                                            response = urllib.request.urlopen(
                                                                str(url)
                                                        ...
                                                        if urls:
                                                            ...
                                                            if tmp:
                                                                username = tmp.group(1)
                                                                link = (
                                                                    "https://kindle.amazon.com/profile/"
                                                                    + urls.group(1)
                                                                )
                                                                try:
                                                                    response = (
                                                                    ...
                                                                    pass
                                                                urls = re.search(
                                                                    '<img alt="(.*)" class="profilePhoto" src="(.*).jpg" />',
                                                                    html,
                                                                )
                                                                if urls:
                                                                    name = urls.group(
                                                    ...
                                                    if (
                                                        ...
                                                        if urls:
                                                            ...
                                                            try:
                                                                ...
                                                                pass
                                                            urls = re.search(
                                                                'href="http://www.youtube.com/user/(.*)"',
                                                                html,
                                                            )
                                                            if urls:
                                                                username = urls.group(1)
                                                            ...
                                                            try:
                                                                ...
                                                                pass
                                                            tmp = re.search(
                                                                ' <img class="appbar-nav-avatar" src="(.*)" title="(.*)" alt="',
                                                                html,
                                                            )
                                                            if urls:
                                                                pic = tmp.group(1)
                                                        ...
                                                        if (
                                                            ...
                                                            #: ----------------------------------------------
                                                            urls = re.search(
                                                                '"url" href="https://plus.google.com/(.*)">',
                                                                html,
                                                            )
                                                            if urls:
                                                                tmp = urls.group(1)
                                                                ...
                                                                try:
                                                                    ...
                                                                    pass
                                                                urls = re.search(
                                                                    'href="https://plus.google.com/(.*)"><meta itemprop="name" content=',
                                                                    html,
                                                                )
                                                                if urls:
                                                                    username = (
                                                                ...
                                                                tmp = re.search(
                                                                    '<meta itemprop="image" content="(.*)"><meta itemprop="url"',
                                                                    html,
                                                                )
                                                                if tmp:
                                                                    pic = (
                                                                ...
                                                                tmp = re.search(
                                                                    '<meta property="og:title" content="(.*): Google\+"><meta name="twitter:title"',
                                                                    html,
                                                                )
                                                                if tmp:
                                                                    name = tmp.group(
                                                            ...
                                                            if (
                                                                ...
                                                                #: ----------------------------------------------
                                                                urls = re.search(
                                                                    '<h1 class="post-info-username"><a class="no-ui" href="http://www.frontback.me/(.*)">(.*)</a></h1>',
                                                                    html,
                                                                )
                                                                if urls:
                                                                    username = (
                                                                ...
                                                                try:
                                                                    ...
                                                                    pass
                                                                tmp = re.search(
                                                                    'data-image-url="(.*)"></span></header><div class="pp-usercard"><h2>(.*)</h2><strong>(.*)</strong><p>(.*)</p><span class="pp-location"><i></i>(.*)</span><a class="button button-label-big button-green" data-deeplink=',
                                                                    html,
                                                                )
                                                                if tmp:
                                                                    pic = tmp.group(1)
    ...
    def set_social_networks(self, status):
        try:
            ...
            if not hasattr(status, "retweeted_status"):
                ...
                if status.source.lower().find("instagram") >= 0 and (
                    self.user_sn[status.user.screen_name][0][0] == ""
                    or self.user_sn[status.user.screen_name][0][0] == "Unknown"
                ):
                    # Instagram user.
                    username, link, pic, name, info = self.get_socialnetwork_userinfo(
                        status, "instagram"
                    )
                    ...
                    if status.source.lower().find("foursquare") >= 0 and (
                        self.user_sn[status.user.screen_name][1][0] == ""
                        or self.user_sn[status.user.screen_name][1][0] == "Unknown"
                    ):
                        # Foursquare user
                        (
                            username,
                            ...
                            name,
                            info,
                        ) = self.get_socialnetwork_userinfo(status, "foursquare")
                    ...
                    if status.source.lower().find("facebook") >= 0 and (
                        self.user_sn[status.user.screen_name][2][0] == ""
                        or self.user_sn[status.user.screen_name][2][0] == "Unknown"
                    ):
                        # Facebook user
                        (
                            username,
                            ...
                            name,
                            info,
                        ) = self.get_socialnetwork_userinfo(status, "facebook")
                        ...
                        if status.source.lower().find("linkedin") >= 0 and (
                            self.user_sn[status.user.screen_name][3][0] == ""
                            or self.user_sn[status.user.screen_name][3][0] == "Unknown"
                        ):
                            # LinkedIn user
                            (
                                username,
                                ...
                                name,
                                info,
                            ) = self.get_socialnetwork_userinfo(status, "linkedin")
                            ...
                            if status.source.lower().find("runkeeper") >= 0 and (
                                self.user_sn[status.user.screen_name][4][0] == ""
                                or self.user_sn[status.user.screen_name][4][0]
                                == "Unknown"
                            ):
                                # Runkeeper user
                                (
                                    ...
                                    name,
                                    info,
                                ) = self.get_socialnetwork_userinfo(status, "runkeeper")
                                ...
                                if status.source.lower().find("flickr") >= 0 and (
                                    self.user_sn[status.user.screen_name][5][0] == ""
                                    or self.user_sn[status.user.screen_name][5][0]
                                    == "Unknown"
                                ):
                                    # Flickr user
                                    (
                                        ...
                                        name,
                                        info,
                                    ) = self.get_socialnetwork_userinfo(
                                        status, "flickr"
                                    )
                                    self.user_sn[status.user.screen_name][5][
                                        0
                                    ] = username
                                    ...
                                    if status.source.lower().find("vine") >= 0 and (
                                        self.user_sn[status.user.screen_name][6][0]
                                        == ""
                                        or self.user_sn[status.user.screen_name][6][0]
                                        == "Unknown"
                                    ):
                                        # Vine user
                                        (
                                        ...
                                        self.user_sn[status.user.screen_name][6][
                                            0
                                        ] = username
                                        self.user_sn[status.user.screen_name][6][
                                            1
                                        ...
                                        if status.source.lower().find(
                                            "periscope"
                                        ) >= 0 and (
                                            self.user_sn[status.user.screen_name][7][0]
                                            == ""
                                            or self.user_sn[status.user.screen_name][7][
                                                0
                                            ]
                                            == "Unknown"
                                        ):
                                            ...
                                            self.user_sn[status.user.screen_name][7][
                                                0
                                            ] = username
                                            self.user_sn[status.user.screen_name][7][
                                                1
                                            ...
                                            if status.source.lower().find(
                                                ...
                                                self.user_sn[status.user.screen_name][
                                                    8
                                                ][0]
                                                == ""
                                                or self.user_sn[
                                                ...
                                                if (
                                                    status.source.lower().find("google")
                                                    >= 0
                                                    and status.text.lower().find(
                                                        "a @youtube"
                                                    )
                                                    >= 0
                                                    and (
...
class Geolocation:
    ...
    def __init__(self):
        try:
            ...
            self.toplocationsdatetime = (
                {}
            )  # store date and time of the user most visited locations
            self.toplocationsstartdate = (
                {}
            )  # store initial date of the user most visited locations
            self.toplocationsenddate = (
                {}
            )  # store final date of the user most visited locations
            self.toplocationsstarttime = (
                {}
            )  # store initial time of the user most visited locations
            self.toplocationsdays = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsendtime = (
                {}
            )  # store final time of the user most visited locations
            self.toplocationsdaysmo = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdaystu = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdayswe = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdaysth = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdaysfr = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdayssa = (
                {}
            )  # store week day of the user most visited locations
            self.toplocationsdayssu = (
                {}
            )  # store week day of the user most visited locations
            self.geo_info = []
    ...
    def set_geolocation_information(self, tweet):
        try:
            ...
            if "media" in tweet.entities:
                ...
                if tweet.entities["urls"]:
                    ...
                    if media.find("instagram") > 0:
                        # Instagram
                        try:
                            response = urllib.request.urlopen(str(media))
                            html = response.read()
            ...
            if add:
                place = splace.decode("utf-8")
                sinfo = (
                    media_url + splace.decode("utf-8") + " " + str(sgeo).decode("utf-8")
                )
                self.geo_info.append(
                    [
                        media_url,
                        place,
                        str(sgeo).decode("utf-8"),
                        str(tweet.created_at.strftime("%m/%d/%Y")),
                        str(tweet.created_at.time()),
                        str(tweet.id),
                        source_app,
                    ]
                )
                ...
                if (
                    len(self.visited_locations) > 0
                    and place
                    in self.visited_locations[len(self.visited_locations) - 1][0]
                ):
                    self.visited_locations[len(self.visited_locations) - 1][
                        1
                    ] = tweet.created_at
                    self.visited_locations[len(self.visited_locations) - 1][
                        2
                    ...
                    delta = (
                        self.visited_locations[len(self.visited_locations) - 1][3]
                        - tweet.created_at
                    )
                    self.visited_locations[len(self.visited_locations) - 1][5] = (
                        delta.days + 1
                    )
                    self.visited_locations[len(self.visited_locations) - 1][
                        6
                    ] = self.toplocations_tweets[place]
    ...
    def set_geofile_information(self, tweet, user):
        try:
            ...
            if tweet_geo:
                ...
                content = (
                    '<table width="100%"><tr><td width="48"><img src="'
                    + user.profile_image_url.encode("utf-8")
                    + '"></td><td bgcolor="#cde4f3"><b>'
                    + user.name.encode("utf-8")
                    + "</b> @"
    ...
    def generates_geofile(self, geofile, parameters):
        ...
        try:
            ...
            header = (
                '<table bgcolor="#000000" width="100%"><tr><td><font color="white"><b>'
                + parameters.program_name
                + " "
                + parameters.program_version
                + '</b></font><td align="right"><font color="white">'
                + parameters.program_author_twitter
                + "</font></td></tr></table>"
            )
            ...
            for info in self.kml_info:
                ...
                # Date + Time
                kml_file_body += (
                    "\t\t<name>" + str(info[4]) + " - " + str(info[5]) + "</name>\n"
                )
    ...
    def set_global_information(self, top):
        try:
            ...
            sort_loc = OrderedDict(
                sorted(list(self.toplocations.items()), key=itemgetter(1), reverse=True)
            )
            sort_top = list(sort_loc.items())[0 : int(top)]
            ...
            for place, value in sort_top:
                ...
                self.toploc.append(
                    [
                        str(value),
                        str(startdate.strftime("%m/%d/%Y")),
                        str(enddate.strftime("%m/%d/%Y")),
                        str(starttime),
                        str(endtime),
                        mo_day,
                        tu_day,
...
class Search_GeoTweets:
    ...
    def __init__(self):
        try:
            ...
            self.user_sn = (
                {}
            )  # social networks used by this Twitter user:{'twitteruser': [[Image], [Instagram User, Image], [Foursquare user, Image], [Facebook user, Image]] }. Example: { 'jlopez', ['julian.lopez', 'julianl', 'ad23', 'http://www.facebook.com/profile.php?id=343844'] }
            self.user_taggeds = (
                {}
            )  # { 'user': ['by', 'media_url', 'tweet', 'author_image', 'date', 'time'] }
    ...
    def set_search_information(
        self, hashtag, mentions, user_images, user_tweets, source, activity, top_words
    ):
        ...
        try:
            ...
            tmp_count = 0
            for status in list(tweepy.Cursor(
                api.search,
                ...
                if (
                    tmp_count == 10
                    or tweets_count == int(ui.tb_tweets_number.text())
                    or tmp_count == int(ui.tb_tweets_number.text())
                ):
                    cursor = ui.tb_messages.textCursor()
                    cursor.movePosition(QtGui.QTextCursor.StartOfLine, 0)
                    cursor.movePosition(
                ...
                if is_valid(status):
                    ...
                    if (
                        len(ui.tb_include_words.text()) > 0
                        or len(ui.tb_not_include_words.text()) > 0
                    ):
                    ...
                    profile_image_url = status.user.profile_image_url_https.replace(
                        "_normal.", "."
                    )
                    self.user_sn[status.user.screen_name] = [
                        [profile_image_url],
                    ...
                    if media:
                        if retweeted:
                            ...
                            try:
                                extended_entities = status.extended_entities["media"][
                                    0
                                ]["video_info"]["variants"]
                            except Exception as e:
                                extended_entities = ""
                        ...
                        for m in medias:
                            ...
                            if str(media_url).find("video_thumb") >= 0:
                                if extended_entities:
                                    ...
                                    sort_vid = OrderedDict(
                                        sorted(videos, key=itemgetter(1), reverse=True)
                                    )
                                    vid_top = list(sort_vid.items())[0:1]
                                    media_type = "Video"
                        ...
                        if retweeted:
                            if status.retweeted_status.entities["urls"]:
                                media = str(
                                    status.retweeted_status.entities["urls"][0][
                                        "expanded_url"
                                    ]
                                )
                        ...
                        if media.find("instagram") > 0:
                            ...
                            try:
                                ...
                                (
                                    ...
                                    owner,
                                    profile_image,
                                ) = get_tagged_users_from_instagram(html)
                                keywords_instagram = get_hashtags_from_instagram(html)
                            ...
                            if media.find("swarmapp") > 0:
                                ...
                                (
                                    ...
                                    owner,
                                    profile_image,
                                ) = get_url_media_from_foursquare(media)
                                user_facebook = ""
                                ...
                                if media_url:
                                    (
                                        ...
                                        user_facebook,
                                        profile_image,
                                    ) = get_url_facebook_from_foursquare(
                                        "https://foursquare.com/" + owner
                                    )
                                    self.user_sn[status.user.screen_name][3] = [
                                        user_facebook,
                                        profile_image,
                                    ]
                    ...
                    self.adv_geo_info.append(
                        [
                            ...
                            str(created_at.time()),
                            status.user.profile_image_url_https.replace(
                                "_normal.", "."
                            ),
                            status.text,
                        ]
    ...
    def set_geolocation_information(
        ...
        try:
            ...
            for status in list(tweepy.Cursor(
                ...
                if hasattr(status, "retweeted_status"):
                    profile_image_url = (
                        status.retweeted_status.author.profile_image_url_https.replace(
                            "_normal.", "."
                        )
                    )
                ...
                if is_valid(status):
                    ...
                    if "media" in status.entities:
                        ...
                        for m in medias:
                            media_url = m["media_url"]
                            if str(media_url).find("video_thumb") >= 0:
                                for content in status.extended_entities["media"][0][
                                    "video_info"
                                ]["variants"]:
                                    if str(content["url"]).find(".mp4") >= 0:
                                        videos.append(
                                            [str(content["url"]), content["bitrate"]]
                                        )
                    ...
                    if self.user_keywords:
                        if status.user.screen_name in self.user_keywords:
...
class Hashtags:
    ...
    def set_global_information(self):
        try:
            ...
            sort_has = OrderedDict(
                sorted(list(self.hashtags_top.items()), key=itemgetter(1), reverse=True)
            )
...
class Mentions:
    ...
    def set_global_information(self):
        try:
            ...
            sort_has = OrderedDict(
                sorted(list(self.mentions_top.items()), key=itemgetter(1), reverse=True)
            )
            self.mentions_top = list(sort_has.items())[0:10]
        ...
        try:
            ...
            if "media" in tweet.entities:
                ...
                for m in medias:
                    media_url = m["media_url"]
                    if str(media_url).find("video_thumb") >= 0:
                        for content in tweet.extended_entities["media"][0][
                            "video_info"
                        ]["variants"]:
                            if str(content["url"]).find(".mp4") >= 0:
...
class User_Conversations:
    ...
    def __init__(self):
        try:
            ...
            self.conversations = (
                {}
            )  # {'tweet1_id1': '[[tweet1], [tweet2], ..., [tweet_n]], 'tweet2_id2': '[[tweet1], [tweet2], ..., [tweet_n]], ...}
    ...
    def make_tweet_to_append(self, tweet, position):
        try:
            ...
            tweet_time = tweet.created_at.time()
            tweet_datetime = (
                tweet.created_at.strftime("%Y/%m/%d") + "-" + str(tweet_time)
            )
                ...
                if tweet.id not in self.processed_tweets:
                    ...
                    if tweet.in_reply_to_status_id:
                        ...
                        try:
                            ...
                            tweet_to_append_in_reply = self.make_tweet_to_append(
                                tweet_in_reply, ""
                            )
                            ...
                            for status in list(tweepy.Cursor(
                                ...
                                if status.in_reply_to_status_id == search_tweet_id:
                                    tweet_to_append = self.make_tweet_to_append(
                                        status, ""
                                    )
                                    self.conversations[conversation_id].append(
                                        tweet_to_append
            ...
            if html:
                ...
                for sname in screenname:
                    if not (sname.lower() in username.lower()):
                        try:
                            b = api.show_friendship(
                                source_screen_name=username, target_screen_name=sname
                            )
                            if b[1].followed_by:
                    ...
                    show_ui_message(
                        str(n)
                        + " identified relations. Followed by: "
                        + str(followedby_count)
                        + " users. Following: "
                        + str(following_count)
                        + " users",
                        "INFO",
                ...
                for content in status:
                    try:
                        ...
                        for i in tweet.entities["hashtags"]:
                            if i["text"] and (
                                i["text"].lower() not in str(hashtags).lower()
                            ):
                        ...
                        for i in tweet.entities["user_mentions"]:
                            ...
                            if (
                                i["screen_name"]
                                and (i["screen_name"].lower() not in username.lower())
                                and (
                                    i["screen_name"].lower()
                                    not in str(mentions).lower()
                                )
                            ):
                                mentions.append(str(i["screen_name"]))
                                b = api.show_friendship(
                                    source_screen_name=username,
                                    target_screen_name=i["screen_name"],
                                )
                                if b[1].followed_by:
                ...
                for sname in followed:
                    if not (sname.lower() in username.lower()) and (
                        sname not in following
                    ):
                        try:
                            b = api.show_friendship(
                                source_screen_name=username, target_screen_name=sname
    ...
    def set_images_information(self, tweet):
        try:
            ...
            if "media" in tweet.entities:
                ...
                for m in medias:
                    ...
                    if str(media_url).find("video") >= 0:
                        ...
                        if (
                            ui.cb_media_download.isChecked()
                            or ui.cb_metadata.isChecked()
                        ):
            ...
            if media_url:
                ...
                screen_name = tweet.user.screen_name
                screen_name_orig = ""
                profile_image_url = tweet.user.profile_image_url_https.replace(
                    "_normal.", "."
                )
                if hasattr(tweet, "retweeted_status"):
                    ...
                    profile_image_url_orig = (
                        tweet.retweeted_status.author.profile_image_url_https.replace(
                            "_normal.", "."
                        )
                    )
                ...
                if os.path.exists(path):
                    ...
                    try:
                        ...
                        longitude = (
                            ...
                            + '"'
                            + " "
                            + str(longitudeRef).split("=")[1][1:-1]
                        )

                        latitude_value = dms_to_decimal(
                            *metadata.__getitem__("Exif.GPSInfo.GPSLatitude").value
                            + [
                        ...
                        longitude_value = dms_to_decimal(
                            *metadata.__getitem__("Exif.GPSInfo.GPSLongitude").value
                            + [
                                metadata.__getitem__(
                                    "Exif.GPSInfo.GPSLongitudeRef"
                                ).value
                        ...
                        self.meta_coordinates[p[0]] = (
                            str(latitude_value) + ", " + str(longitude_value)
                        )
                    except Exception as e:
                        # No GPS information
                        pass
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                try:
                    ...
                    latitude_value = dms_to_decimal(
                        *metadata.__getitem__("Exif.GPSInfo.GPSLatitude").value
                        + [metadata.__getitem__("Exif.GPSInfo.GPSLatitudeRef").value]
                    )
                    longitude_value = dms_to_decimal(
                        *metadata.__getitem__("Exif.GPSInfo.GPSLongitude").value
                        + [metadata.__getitem__("Exif.GPSInfo.GPSLongitudeRef").value]
                    )
    ...
    try:
        ...
        if valid and len(ui.tb_source_app.text()) > 0:
            ...
            if (source_app in tweet.source.lower()) and ui.rb_sourceapp_no.isChecked():
                valid = 0
            if (
                source_app not in tweet.source.lower()
            ) and ui.rb_sourceapp_yes.isChecked():
                valid = 0
...
def generates_HTML_file(
    ...
    try:
        ...
        jinja2_env = Environment(
            loader=FileSystemLoader(tinfoleak_dir), autoescape=True, trim_blocks=True
        )
        ...
        for tid, conv in list(user_conversations.conversations.items()):
        ...
        template_values = {
            ...
            # source
            "output_source": (
                ui.cb_source_apps.isChecked() and ui.cb_source_apps.isEnabled()
            ),
            ...
            # social networks
            "output_social": (
                ui.cb_social_networks.isChecked() and ui.cb_social_networks.isEnabled()
            ),
            "social_tweet": social.user_sn,
            "social_results": len(social.user_sn),
            # hashtag
            "output_hashtag": (
                ui.cb_hashtags.isChecked() and ui.cb_hashtags.isEnabled()
            ),
            ...
            # mention
            "output_mention": (
                ui.cb_mentions.isChecked() and ui.cb_mentions.isEnabled()
            ),
            ...
            # find text
            "output_tweet": (
                ui.cb_show_tweets.isChecked() and ui.cb_show_tweets.isEnabled()
            ),
            "find": "",
            ...
            # meta
            "output_metadata": (
                ui.cb_metadata.isChecked() and ui.cb_metadata.isEnabled()
            ),
            ...
            # geolocation
            "output_geolocation": (
                ui.cb_visited_locations.isChecked()
                and ui.cb_visited_locations.isEnabled()
            ),
            ...
            # conversations
            "output_conversation": (
                ui.cb_conversations.isChecked() and ui.cb_conversations.isEnabled()
            ),
            "conversations": list(user_conversations.conversations.items()),
            ...
            # top words
            "output_words": (
                ui.cb_words_frequency.isChecked() and ui.cb_words_frequency.isEnabled()
            ),
            ...
            # activity
            "output_activity": (
                ui.cb_activity.isChecked() and ui.cb_activity.isEnabled()
            ),
            ...
            # protected account
            "output_protected": (
                ui.cb_protected_account.isChecked()
                and ui.cb_protected_account.isEnabled()
            ),
        ...
        html_content = jinja2_env.get_template(
            "ReportTemplate/tinfoleak-theme.html"
        ...
        output_report_filename = (
            parameters.html_output_directory + "/" + str(ui.tb_report_filename.text())
        )
...
def get_information_for_user_target():
    try:
        ...
        img_directory = (
            os.path.dirname(os.path.abspath(__file__)) + "/" + user.screen_name
        )
        ...
        try:
            ...
            img = urllib.request.urlopen(
                user.profile_image_url.replace(".jpg", "_400x400.jpg")
            ).read()
        filename = str(user.id) + ".jpg"
        ...
        datenow, timenow = show_ui_message(
            "<img src=" + imgFile + " height=60 align=middle>", "INFO", 1
        )
        ...
        if (
            ui.cb_source_apps.isChecked()
            or ui.cb_activity.isChecked()
            or ui.cb_hashtags.isChecked()
            or ui.cb_mentions.isChecked()
            or ui.cb_words_frequency.isChecked()
            or ui.cb_social_networks.isChecked()
            or ui.cb_visited_locations.isChecked()
            or ui.cb_metadata.isChecked()
            or ui.cb_media.isChecked()
            or ui.cb_conversations.isChecked()
            or ui.cb_show_tweets.isChecked()
        ):
            page = 1
            tweets_count = 0
            tmp_count = 0
            while True:
                timeline = api.user_timeline(
                    screen_name=username, include_rts=True, count=10, page=page
                )
                if timeline:
        ...
        if results:
            ...
            if ui.cb_followers.isChecked():
                ...
                if not ui.tb_followers_number.text():
                    ...
                    followers.get_followers(
                        username, api, int(ui.tb_followers_number.text())
                    )
            ...
            if ui.cb_words_frequency.isChecked():
                ...
                if not ui.tb_words_frequency_number.text():
                    ...
                    wordlist = sorted(
                        list(top_words.top_words.items()), key=operator.itemgetter(1)
                    )
                    wordlist.reverse()
            ...
            if ui.cb_metadata.isChecked():
                ...
                if user.profile_image_url.find("_normal") < 0:
                    tmp_profile_image_url = user.profile_image_url.replace(
                        ".jpg", "_400x400.jpg"
                    )
                else:
                    tmp_profile_image_url = user.profile_image_url.replace(
        ...
        if ui.cb_likes.isChecked():
            ...
            if user.favourites_count:
                if not ui.tb_likes_number.text():
                    ...
                    favorites.set_favorites_information(
                        api, username, int(ui.tb_likes_number.text())
                    )
...
def get_information_for_place():
    ...
    try:
        ...
        activity = Activity()
        coordinates = (
            ui.tb_place_lat.text()
            + ","
            + ui.tb_place_lon.text()
            + ","
            + ui.tb_place_km.text()
            + "km"
        )
        ...
        results = search.set_geolocation_information(
            coordinates,
            hashtag,
            mentions,
            social_networks,
...
def get_information_for_timeline():
    ...
    try:
        ...
        results = search.set_search_information(
            hashtag, mentions, user_images, user_tweets, source, activity, top_words
        )
...
def get_hashtags_from_instagram(html):
    ...
    try:
        ...
        hash = re.findall(
            '<meta content="(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+" property="instapp:hashtags"',
            html,
        )
        for h in hash:
...
def get_url_media_from_foursquare(url):
    ...
    try:
        ...
        urls = re.search(
            '<div id="mapContainer"><img src="(.*)" alt="(.*)" class="avatar mainUser"',
            html,
        )
        if urls:
            ...
            if urls:
                ...
                if urls:
                    ...
                    try:
                        ...
                        owner = urls.group(1)[
                            last_position + 3 : len(urls.group(1)) - 2
                        ]
...
def get_url_facebook_from_foursquare(url):
    ...
    try:
        ...
        urls = re.search(
            '<a href="http://www.facebook.com/(.*)" rel="nofollow" target="_blank" class="fbLink iconLink"',
            html,
        )
        if urls:
...
def get_user_from_facebook_url(url):
    ...
    try:
        ...
        if urls:
            ...
            try:
                response = urllib.request.urlopen("http://longurl.org/expand?url=" + url)
                html = response.read()
                urls = re.search(
                    '<a href="https://www.facebook.com/(.*)/posts/[0-9]*">https://',
                    html,
...
def get_user_from_flickr_url(url):
    ...
    try:
        ...
        urls = re.search(
            '"og:url" content="https://www.flickr.com/photos/(.*)/[0-9]*/"', html
        )
        if urls:
...
def get_user_from_googleplus_url(url):
    ...
    try:
        ...
        if urls:
            ...
            urls = re.search(
                '<meta itemprop="url" content="https://plus.google.com/(.*)"><link rel="alternate" href="android-app:',
                html,
            )
            if urls:
                user = tmp + "/" + urls.group(1)
...
def get_user_from_frontback_url(url):
    ...
    try:
        ...
        urls = re.search(
            'post-info-username"><a class="no-ui" href="http://www.frontback.me/(.*)">(.*)</a></h1><h2 class="post-info-caption',
            html,
        )
        if urls:
            user = urls.group(1)
...
def get_url_media_from_youtube(url):
    ...
    try:
        ...
        urls = re.findall(
            'data-expanded-url="http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+',
            html,
        )
            ...
            if table.cellWidget(rowPosition, checkbox_column):
                if (
                    ...
                    ui.tb_username.setText(
                        table.item(rowPosition, screen_name_column)
                        .text()
                        .replace("@", "")
                    )
                    ...
                    try:
                        if row < users_limit:
                            ...
                            if len(tmp_user.profile_image_url) > 0:
                                img = urllib.request.urlopen(
                                    tmp_user.profile_image_url.replace("_normal.", ".")
                                ).read()
                            else:
                                img = urllib.request.urlopen(
                            ...
                            try:
                                img = urllib.request.urlopen(
                                    tmp_user.profile_banner_url.replace("_normal.", ".")
                                ).read()
                            except Exception as e:
                                if tmp_user.profile_background_image_url:
                                    img = urllib.request.urlopen(
                                        tmp_user.profile_background_image_url.replace(
                                            "_normal.", "."
                                        )
                                    ).read()
                            ...
                            icon_button_profile_background_image.setIconSize(
                            ...
                            users_window_ui.tbl_users.setCellWidget(
                                row, 1, icon_button_profile_image
                            )
                            users_window_ui.tbl_users.setCellWidget(
...
def showUserRelations():
    try:
        ...
        # Connect pushButton
        user_relations_ui.pb_select_file_user2.clicked.connect(
            lambda: selectUsersFile(user_relations_ui.lb_file_user2)
        )
...
def get_icon_button_profile_image(tmp_user, tmp_window):
    try:
        ...
        img_directory = (
            os.path.dirname(os.path.abspath(__file__)) + "/" + tmp_user.screen_name
        )
        if not os.path.isdir(img_directory):
            os.mkdir(img_directory)
...
def get_icon_button_relations_image(user1, user2, tmp_window):
    try:
        ...
        friendship = api.show_friendship(
            source_screen_name=user1, target_screen_name=user2
        )

        # Arrow directory
        img_directory = (
            os.path.dirname(os.path.abspath(__file__)) + "/Output_Reports/img"
        )
            ...
            for line in f.readlines():
                try:
                    if row < int(ui.tb_users_number.text()):
                        ...
                        if (
                            str(tmp_user1.screen_name).lower()
                            != str(tmp_user2.screen_name).lower()
                        ):
                            ...
                            icon_button_profile_image_user1 = (
                                get_icon_button_profile_image(
                                    tmp_user1, user_relations_window
                                )
                            )
                            icon_button_profile_image_user2 = (
                                get_icon_button_profile_image(
                                    tmp_user2, user_relations_window
                                )
                            )
                            ...
                            (
                                icon_button_relations_image,
                                relation_code,
                            ) = get_icon_button_relations_image(
                                username, tmp_user2.screen_name, user_relations_window
                            )
...
def show_lists():
    try:
        ...
        icon_button_profile_image_user = get_icon_button_profile_image(
            tmp_user, user_lists_window
        )
...
def show_collections():
    try:
        ...
        tmp_user = api.get_user(screen_name)
        icon_button_profile_image_user = get_icon_button_profile_image(
            tmp_user, user_collections_window
        )
...
def show_followers():
    try:
        ...
        tmp_user = api.get_user(screen_name)
        icon_button_profile_image_user = get_icon_button_profile_image(
            tmp_user, user_followers_window
        )
        ...
        if not os.path.isdir(username_directory):
            show_ui_message(
                "Resource <b>" + username_directory + "</b> not found", "ERROR", 1
            )
...
def selectTargetFromUserRelations():
    try:
        ...
        selection = selectUser(
            table=user_relations_ui.tbl_relations,
            checkbox_column=0,
            screen_name_column=2,
        )
        if not selection:
            selectUser(
                table=user_relations_ui.tbl_relations,
                checkbox_column=5,
                screen_name_column=7,
            )
    ...
    try:
        ...
        if user1 != "" and user2 != "":
            ...
            if user_relations_ui.cb_profile_images.isChecked():
                # User 1
                icon_button_profile_image_user1 = get_icon_button_profile_image(
                    tmp_user1, user_relations_window
                )

                # User 2
                icon_button_profile_image_user2 = get_icon_button_profile_image(
                    tmp_user2, user_relations_window
                )

                user_relations_ui.tbl_relations.setCellWidget(
                    0, 1, icon_button_profile_image_user1
                )
                user_relations_ui.tbl_relations.setCellWidget(
                    0, 6, icon_button_profile_image_user2
                )
            ...
            (
                icon_button_relations_image,
                relation_code,
            ) = get_icon_button_relations_image(user1, user2, user_relations_window)
            ...
            if user1 != "" and file2 != "":
                show_relation_from_user_to_file(
                    user1, file2, user_relations_ui.tbl_relations
                )
...
if __name__ == "__main__":
    try:
        ...
        w = QtWidgets.QDialog(parent=window)
        users_window_ui = users_window.Ui_Dialog()
        users_window_ui.setupUi(w)
        # Connect buttonBox
        btn = users_window_ui.buttonBox.button(QtWidgets.QDialogButtonBox.Ok)
        btn.clicked.connect(
            lambda: selectUser(
                table=users_window_ui.tbl_users, checkbox_column=0, screen_name_column=4
            )
        )
        # Connect checkbox : select all
        users_window_ui.cb_all.stateChanged.connect(
            lambda: setUnsetAllUsers(
                table=users_window_ui.tbl_users,
                checkbox_all=users_window_ui.cb_all,
                column=0,
            )
        )

        # Window to identify user relations
        user_relations_window = QtWidgets.QDialog(parent=window)
        ...
        user_relations_ui.cb_all_user1.stateChanged.connect(
            lambda: setUnsetAllUsers(
                table=user_relations_ui.tbl_relations,
                checkbox_all=user_relations_ui.cb_all_user1,
                column=0,
            )
        )
        user_relations_ui.cb_all_user2.stateChanged.connect(
            lambda: setUnsetAllUsers(
                table=user_relations_ui.tbl_relations,
                checkbox_all=user_relations_ui.cb_all_user2,
                column=5,
            )
        )
        user_relations_ui.cb_relations.stateChanged.connect(
            lambda: setUnsetUserRelations(
                table=user_relations_ui.tbl_relations,
                relation_column=9,
                checkbox_relation=user_relations_ui.cb_relations,
            )
        )
